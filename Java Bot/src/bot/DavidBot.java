...
public class DavidBot {
    ...
    public DavidBot(String name, int stackSize, int bigBlind, int numHands, double timeBank, List<Player> otherPlayers){
    ...
    public String getAction(String legalActionsArray[]){
        ...
        if(boardCards.getStreet() == Street.PREFLOP){
        ...
        else if(boardCards.getStreet() == Street.FLOP){
            ActionProbability actionProb = flopStrategy(legalActions);
            LegalActionType actionTypeToPerform = actionProb.randomlyChooseAction();
            System.out.println("action to perform: " + actionTypeToPerform);
            ...
            ActionProbability actionProb = turnStrategy(legalActions);
            LegalActionType actionTypeToPerform = actionProb.randomlyChooseAction();
            System.out.println("action to perform: " + actionTypeToPerform);
            ...
            ActionProbability actionProb = riverStrategy(legalActions);
            LegalActionType actionTypeToPerform = actionProb.randomlyChooseAction();
            System.out.println("action to perform: " + actionTypeToPerform);
    ...
    private ActionProbability flopStrategy(Map<LegalActionType, LegalAction> legalActions){
        ...
        if(potOdds*1.4<equity){
            ...
            return new ActionProbability(0, 0.3, .7, 0, 0);
        }
        else if(potOdds<equity){
            //fold, call, raise, bet, check
            return new ActionProbability(0, 0.7, .3, 0, 0);
        }
        else{
            return new ActionProbability(0.5, 0.25, 0.25, 0, 0);
        }
    }
    
    private ActionProbability turnStrategy(Map<LegalActionType, LegalAction> legalActions){
        //get the call amount, call amount is 0 if call is not a legal action
        int callAmount = 0;
        if(legalActions.containsKey(LegalActionType.CALL)){
            callAmount=legalActions.get(LegalActionType.CALL).getAmount();
        ...
        if(potOdds*1.4<equity){
            ...
            return new ActionProbability(.7, .15, .15, 0, 0);
        }
        
    }
    
    private ActionProbability riverStrategy(Map<LegalActionType, LegalAction> legalActions){
        //get the call amount, call amount is 0 if call is not a legal action
        int callAmount = 0;
        if(legalActions.containsKey(LegalActionType.CALL)){
            callAmount=legalActions.get(LegalActionType.CALL).getAmount();
        ...
        if(potOdds*1.4<equity){
            ...
            return new ActionProbability(.9, .05, .05, 0, 0);
        }
    }
}
